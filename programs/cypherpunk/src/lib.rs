...
declare_id!("GYerhyLnbdDZdyvLMz8P2MLspHo9LHmi4V7F1XGYyScF");
...
pub mod cypherpunk {
    ...
    pub fn swap(
        ctx: Context<Swap>,
        computation_offset: u64,
        ciphertext_amount_in: [u8; 32],
        ciphertext_rate_ppm: [u8; 32],
        pub_key: [u8; 32],
        nonce: u128,
    ) -> Result<()> {
        ctx.accounts.sign_pda_account.bump = ctx.bumps.sign_pda_account;
        let args = vec![
            Argument::ArcisPubkey(pub_key),
            Argument::PlaintextU128(nonce),
            Argument::EncryptedU64(ciphertext_amount_in),
            Argument::EncryptedU64(ciphertext_rate_ppm),
        ];
    ...
    pub fn swap_callback(
        ...
        let o = match output {
            ComputationOutputs::Success(SwapOutput { field_0 }) => field_0,
            _ => return Err(ErrorCode::AbortedComputation.into()),
        };

        emit!(SwapEvent {
            amount_out: o.ciphertexts[0],
            nonce: o.nonce.to_le_bytes(),
        });
        Ok(())
    }
